...
most_popular_in_year <- function(years) {
  name_in_year <- names[names$year == years, ]
  most_popular <- name_in_year[name_in_year$prop == max(name_in_year$prop), "name"]
  most_popular
}
...
number_in_a_millions <- function(name, years) {
  name_for_year <- names[names$year == years, ]
  percentage <- name_for_year[name_for_year$name == name, "prop"]
  percentage <- percentage * 1000000
  percentage
}

# How many babies out of 1 million had the name 'Laura' in 1995?
number_in_a_millions("Laura", 1995)

# How many babies out of 1 million had your name in the year you were born?
number_in_a_millions("Kai", 1997)
